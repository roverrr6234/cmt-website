 ...
 * 5대 서비스 시드 스크립트.
 *
 * client/src/lib/serviceData.ts 의 5개 서비스를 Sanity 의 service 도큐먼트로 동기화한다.
 * createOrReplace 를 사용하므로 이미 존재하는 서비스는 덮어쓴다 — 5대 서비스는 항상
 * 코드의 진실(source of truth)을 따른다는 정책에 부합한다.
 *
 * 실행:
 *   node scripts/seed-services.mjs
 *
 * 토큰은 ../.env (sanity-studio/.env) 의 SANITY_AUTH_TOKEN 에서 자동 로드.
 */
...
if (!TOKEN) throw new Error("Missing SANITY_AUTH_TOKEN in sanity-studio/.env");

/* ── 헬퍼 ── */
const k = () => randomUUID().replace(/-/g, "").slice(0, 12);

/** 평문 텍스트 → Portable Text 단일 블록 */
const pt = (text) => [
  {
    _type: "block",
    _key: k(),
...
/** Procedure step */
const step = (s, d) => ({ _type: "procedureStep", _key: k(), step: s, detail: d });

/** Section: text */
const sText = (title, content) => ({
  _type: "sectionText",
  _key: k(),
  title,
  content: pt(content),
...
/** Section: alert */
const sAlert = (title, alertType, alertTitle, alertContent) => ({
  _type: "sectionAlert",
  _key: k(),
  title,
  alertType,
...
/** Section: table — rows is array of cell arrays */
const sTable = (title, headers, rows, footnote, tableTitle) => ({
  _type: "sectionTable",
  _key: k(),
  title,
  ...(tableTitle && { tableTitle }),
  headers,
  rows: rows.map((cells) => ({ _type: "row", _key: k(), cells })),
  ...(footnote && { footnote }),
});

/** Section: checklist */
const sChecklist = (title, listTitle, items) => ({
  _type: "sectionChecklist",
  _key: k(),
  title,
  listTitle,
  items,
...
/** Section: comparison table — rows is array of {label, values} */
const sCompare = (title, headers, rows, footnote, tableTitle) => ({
  _type: "sectionComparisonTable",
  _key: k(),
  title,
  ...(tableTitle && { tableTitle }),
  headers,
  rows: rows.map((r) => ({ _type: "comparisonRow", _key: k(), label: r.label, values: r.values })),
  ...(footnote && { footnote }),
});

/** Section: procedure image (with diagramId for code-rendered diagrams in client/src/components/diagrams) */
const sDiagram = (title, diagramId) => ({
  _type: "sectionProcedureImage",
  _key: k(),
  title,
  diagramId,
});
...
const preventionPlan = {
  _id: "service-prevention-plan",
  _type: "service",
  title: "화학사고 예방관리 계획서",
  shortTitle: "화학사고예방관리계획서",
  slug: { _type: "slug", current: "prevention-plan" },
  iconName: "FileText",
  ...
  procedure: [
    step("사전 상담", "사업장 현황 파악 및 제출 대상 여부 판별"),
    step("자료 수집", "취급 화학물질 목록, 시설 정보, 입지 정보 수집"),
    step("계획서 작성", "기본정보, 시설정보, 장외평가정보, 비상대응계획 작성"),
    step("사고시나리오 평가", "사고시나리오 선정 및 영향범위 평가 수행"),
    step("제출 및 심사", "화학물질안전원 제출 및 심사 대응"),
    step("적합 통보", "적합 통보 수령 및 사후 관리 안내"),
  ],
  documents: [
    "사업장 일반정보 및 취급시설 개요",
    "유해화학물질 목록 및 유해성 정보",
  ...
  sortOrder: 1,
  sections: [
    sText("제출 방법", "취급시설의 검사 개시일 60일 이전에 신청서, 화학사고예방관리계획서를 화학물질안전원장에게 제출합니다."),
    sChecklist("참고사항", "제출 시 유의사항", [
    ...
    sDiagram("제출수준 판정 절차", "judge-flow"),
    sDiagram("최대보유량 산정", "max-calc"),
    sTable(
      "최대보유량 산정 기준",
      ["구분", "주요사항", "비고"],
      [
        ["공통", "정의: 각 물질 취급시설에서 최대 체류할 수 있는 양의 합\n방법: '설계용량', '비중' 고려\n공식: 물질별 최대보유량(톤) = Σ(설계용량×비중)", "혼합형태 존재 시 혼합물 비중 고려가능 (증빙 가능한 경우에 한함)"],
        ["제조·사용시설", "• '설계용량', '비중' 고려\n• 각각의 성상이 차지하는 '부피'로 산정 가능\n• 운전조건(온도, 압력)을 고려 산정 가능\n• 저장방식(압축 등) 및 성상 고려 산정 가능", "• 함량기준 이상 존재시\n• 성상이 다를 시(증빙 가능한 경우에 한함)\n• 가체물질의 경우\n• 액화가스, 압축가스의 경우"],
        ["저장·보관시설", "• '설계용량', '비중' 고려\n• 제출한 '보관 구획도' 기준으로 산정", "• 저장시설\n• 보관시설"],
      ],
      undefined,
      "최대보유량 산정 기준",
    ),
    sChecklist("제출 면제시설", "화학사고예방관리계획서 제출 면제 대상", [
    ...
    sDiagram("구성요소 (1군 vs 2군 비교)", "compare-1-2"),
    sCompare(
      "구성요소 상세 비교",
      ...
      undefined,
      "화학사고예방관리계획서 구성요소 (1군 vs 2군)",
    ),
    sChecklist("변경 제출 사유", "변경 제출이 필요한 경우", [
 ...
 * 2. 유해화학물질 취급시설 설치검사
 * ══════════════════════════════════════════════════════════════════════════ */
const installationInspection = {
  _id: "service-installation-inspection",
  _type: "service",
  title: "유해화학물질 취급시설 설치검사",
  shortTitle: "유해화학물질 취급시설 설치검사",
  slug: { _type: "slug", current: "installation-inspection" },
  iconName: "Search",
  ...
  penalty: "미검사 가동 시 5,000만원 이하의 과태료 / 검사 거부·방해·기피 시 1,000만원 이하의 과태료",
  tasks: [
    "사전 서면검토 및 도면 검토",
    "현장 가동 전 설치검사 수검 지원",
    "부적합 사항 보완 컨설팅",
  ...
  procedure: [
    step("계약완료", "EHS 데이터 모집"),
    step("현장진단 및 진단보고서 제출", "사이트 등록 / 산세 체크 → 현장 개선방안 제시"),
    step("현장개선 및 서면자료 작성", "최종 수량 계산 목서"),
    step("검사 수검 (검사 기관)", "검사기관에 의한 설치·정기·수시검사 수행"),
    step("검사완료 (영업허가 진행)", "적합 판정 수령 및 영업허가 절차 진행"),
  ],
  documents: [
  ...
  sortOrder: 2,
  sections: [
    sAlert("검사기관", "info", "유해화학물질 취급시설 검사기관", "한국환경공단, 한국가스안전공사, 한국산업안전보건공단"),
    sText(
      "업무 개요",
    ...
    sDiagram("유해화학물질 취급시설의 종류", "facility-types"),
    sDiagram("검사 및 안전진단 시기/주기", "inspection-schedule-img"),
    sTable(
      "검사 및 안전진단 시기/주기 상세",
      ["구분", "대상/조건", "시기 / 주기"],
      [
    ...
    sDiagram("컨설팅 절차", "consulting-flow-inspection"),
    sChecklist("관련 법규", "업무 수행 근거 법규", [
...
const businessLicense = {
  _id: "service-business-license",
  _type: "service",
  title: "유해화학물질 영업허가",
  shortTitle: "유해화학물질 영업허가",
  slug: { _type: "slug", current: "business-license" },
  iconName: "Award",
  ...
  procedure: [
    step("사전 상담", "영업 유형 확인 및 허가 요건 검토"),
    step("서류 준비", "필요 서류 목록 안내 및 작성 지원"),
    step("사전 검토", "제출 서류 적합성 사전 검토"),
    step("신청 접수", "화관법 민원24 또는 지방환경관서 접수"),
    step("현장 확인", "지방환경관서 현장 확인 대응"),
    step("허가 취득", "영업허가증 수령 및 사후 관리"),
  ],
  documents: [
  ...
  sections: [
    ...
    sChecklist("영업허가 면제 대상", "화학물질관리법 제29조에 따른 영업허가 면제 대상", [
...
const psm = {
  ...
  penalty: "미제출 시 1,000만원 이하의 과태료 / 이행상태 미흡 시 공정중지명령",
  tasks: [
    "공정위험성평가 수행",
    "사고조사 및 비상조치계획 수립",
    "등급 관리(P/S/M/C) 컨설팅",
  ...
  procedure: [
    step("계약 완료", "대상 물질 및 규정량 확인, 제출 대상 판별"),
    step("현장 진단 및 진단보고서 제출", "현장 개선방안 제시"),
    step("현장 개선 및 서면자료 작성", "공정안전자료, 위험성평가, 안전운전계획, 비상조치계획 작성"),
    step("서류 제출 및 심사 수검 (안전보건공단)", "KOSHA 심사 대응"),
    step("심사 결과 보고 (고용노동부)", "MOEL 최종 결과 보고"),
  ],
  documents: [
    "공정안전자료 (공정흐름도, P&ID 등)",
    "공정위험성 평가서",
  ...
  sortOrder: 4,
  sections: [
    sAlert("작성 자격", "info", "공정안전보고서(PSM) 작성 자격", "공정안전보고서(PSM) 작성 자격을 갖춘 자가 작성해야 합니다."),
    sTable(
      "제출 정보",
      ["제출시기", "제출서류", "제출방법"],
      [
    ...
    sDiagram("PSM 주요 구성 요소 및 플로우", "psm-components"),
    sDiagram("PSM 컨설팅 절차", "psm-consulting"),
  ],
};
 ...
 * 5. 유해위험방지계획서
 * ══════════════════════════════════════════════════════════════════════════ */
const hazardPrevention = {
  _id: "service-hazard-prevention",
  _type: "service",
  title: "유해위험방지계획서",
  shortTitle: "유해위험방지계획서",
  slug: { _type: "slug", current: "hazard-prevention" },
  ...
  procedure: [
    step("1단계: 계약 완료", "대상 업종 및 설비 확인, 제출 대상 판별"),
    step("2단계: 현장 방문 및 필요자료 요청", "현장 개선방안 제시"),
    step("3단계: 현장 개선 및 서면 자료 작성", "유자격자에 의한 계획서 작성"),
    step("4단계: 서류 제출 및 심사 수검 (안전보건공단)", "KOSHA 심사 대응"),
    step("5단계: 심사 결과 교부", "사업장에 최종 결과 전달"),
  ],
  documents: [
  ...
  sections: [
    ...
    sChecklist("제출대상 5개 설비", "모든 업종의 사업장에서 노동부령으로 정하는 5개 설비를 설치·이전·변경하는 경우", [
    ...
    sDiagram("심사 및 확인절차", "review-process"),
    sChecklist("심사 및 확인 안내", "심사 및 확인 절차 안내", [
      "심사절차: 제출된 유해·위험방지계획서의 적합성 및 타당성을 검토하여 접수일로부터 15일 이내 심사결과 교부",
      "확인일: 유해·위험방지계획서 제출시 작성한 시운전기간 중 현장방문하여 계획서와 현장일치 여부 확인 후 결과 교부",
    ]),
    sDiagram("컨설팅 절차", "consulting-flow-hazard"),
  ],
};

/* ── 시드 실행 ── */
const services = [preventionPlan, installationInspection, businessLicense, psm, hazardPrevention];

/**
 * 기존 옛 ID(`service.xxx` — Sanity 가 anonymous 차단) 도큐먼트가 남아있다면 삭제한 뒤
 * 새 ID(`service-xxx`) 로 createOrReplace.
 */
const legacyIds = [
  "service.prevention-plan",
  "service.installation-inspection",
  "service.business-license",
  "service.psm",
  "service.hazard-prevention",
  "test-service-doc-1",
];

const mutations = [
  ...legacyIds.map((id) => ({ delete: { id } })),
  ...services.map((doc) => ({ createOrReplace: doc })),
];

console.log(`Seeding ${services.length} services to https://${PROJECT_ID}.api.sanity.io/${DATASET} ...`);

const resp = await fetch(
  `https://${PROJECT_ID}.api.sanity.io/v2024-12-01/data/mutate/${DATASET}?returnIds=true`,
  {
    method: "POST",
    headers: {
